...
def bayes_glm_init(network_structures, x, y):
    ...
    n_sample = len(y)
    batchs = [j for j in batch(range(0, n_sample), int(
        n_sample / np.prod(network_structures)) + 10)]

    batch_index = 0
    weights_list = []
    for layer_index in range(len(network_structures) - 1):
        init_weights = np.zeros(
            [network_structures[layer_index], network_structures[layer_index + 1]])
        for out_features in range(network_structures[layer_index + 1]):
            ...
            columns_names = [
                'x' + str(i) for i in range(network_structures[layer_index])]
...
def plot_traces(traces, retain=0):
    ...
    for i, mn in enumerate(pm.summary(traces[-retain:])['mean']):
        ax[i, 0].annotate('{:.2f}'.format(mn), xy=(mn, 0), xycoords='data', xytext=(
            5, 10), textcoords='offset points', rotation=90, va='bottom', fontsize='large', color='#AA0022')
...
def batch_trainning(x_train, y_train, x_test, y_test, B, S, network_structures, epislon=3, mu=0, sigma=1.5, mu_p=0.0, sigma_p=0.1, lr=1, lr_s=0.02, verbose=False, bayesian_init=True):
    ...
    if bayesian_init == True:
        mu_vector1, mu_vector2, mu_vector3 = bayes_glm_init(
            network_structures, x_train, y_train)
    else:
    ...
    for j in batch(range(0, len(y_train)), B):
        ...
        args = list(zip(list(range(S)), [mu_vector1] * S, [mu_vector2] * S, [mu_vector3] * S, [
                    sigma_vector1] * S, [sigma_vector2] * S, [sigma_vector3] * S, [x] * S, [y] * S))
        ...
        lc_sigma1 = ada_sigma1 * 0.5 / B * \
            (1 / sigma_vector1**2 - 1 / sigma_p**2)
        lc_sigma2 = ada_sigma2 * 0.5 / B * \
            (1 / sigma_vector2**2 - 1 / sigma_p**2)
        lc_sigma3 = ada_sigma3 * 0.5 / B * \
            (1 / sigma_vector3**2 - 1 / sigma_p**2)
       ...
       #     print ('Entered!!!')
        ...
        mu_p = np.mean(np.append(np.append(mu_vector1.ravel(),
                                           mu_vector2.ravel()), mu_vector3.ravel()))
    ...
    # sigma_p update
        ...
        sigma_p = np.sqrt((sigma_p1 + sigma_p2 + sigma_p3) /
                          (para_num * M1 + M1 * M2 + M2 * 1))

        Lc1 = np.sum(np.log(sigma_vector1 / sigma_p1) + 0.5 / sigma_p **
                     2 * ((mu_vector1 - mu_p)**2 + sigma_vector1**2 - sigma_p**2))
        Lc2 = np.sum(np.log(sigma_vector2 / sigma_p2) + 0.5 / sigma_p **
                     2 * ((mu_vector2 - mu_p)**2 + sigma_vector2**2 - sigma_p**2))
        Lc3 = np.sum(np.log(sigma_vector3 / sigma_p3) + 0.5 / sigma_p **
                     2 * ((mu_vector3 - mu_p)**2 + sigma_vector3**2 - sigma_p**2))
        ...
        if np.mean(-Lc / B + Le) >= max(elbo):
            best_arg = list(zip(list(range(S)), [mu_vector1] * S, [mu_vector2] * S, [mu_vector3] * S, [
                            sigma_vector1] * S, [sigma_vector2] * S, [sigma_vector3] * S, [x_test] * S, [y_test] * S))
        if verbose:
...
def results_plot(best_arg):
    ...
    plt.fill_between(range(len(pred_prob)), prob_ub,
                     prob_lb, color='b', alpha=.1)
    plt.show()
...
#    M1=M2=4
    ...
    best_arg = batch_trainning(x_train, y_train, x_test, y_test, B, S, network_structures,
                               epislon=3, mu=0, sigma=1.5, mu_p=0.0, sigma_p=1.6, lr=1, lr_s=0.01)
